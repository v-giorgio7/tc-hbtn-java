

public class Program {
    public static void main(String[] args) {
        Funcionario funcionario1 = new Funcionario(2360);

        double salarioLiquido90porcento = FolhaPagamento.calcularLiquido(funcionario1, x -> x * 0.9);
        double salarioLiquidoAcrescimo50 = FolhaPagamento.calcularLiquido(funcionario1, x -> x + 50);
        double salarioLiquidoAcrescimo10porcento = FolhaPagamento.calcularLiquido(funcionario1, x -> x * 1.1);

        System.out.printf("%s - %s\n", convertDouble(funcionario1.getSalario()), convertDouble(salarioLiquido90porcento));
        System.out.printf("%s - %s\n", convertDouble(funcionario1.getSalario()), convertDouble(salarioLiquidoAcrescimo50));
        System.out.printf("%s - %s\n", convertDouble(funcionario1.getSalario()), convertDouble(salarioLiquidoAcrescimo10porcento));
    }

    private static String convertDouble(Double valor) {
        String valorStr = String.format("%.2f", valor);
        return valorStr.replace(',', '.');
    }
}
